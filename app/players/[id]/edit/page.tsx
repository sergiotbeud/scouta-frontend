...
export default function EditPlayerPage() {
    ...
    const fetchPlayer = async () => {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          setValue('positions', playerData.positions);
          setValue('age', playerData.age);
          setValue('height', playerData.height);
          setValue('weight', playerData.weight);
          setValue('phone', playerData.phone || null);
          setValue('email', playerData.email || null);
          setValue('eps', playerData.eps || null);
          setValue('address', playerData.address || null);
          setValue('emergencyContactName', playerData.emergencyContactName || null);
          setValue('emergencyContactPhone', playerData.emergencyContactPhone || null);
          setValue('emergencyContactRelation', playerData.emergencyContactRelation || null);
  ...
  const onSubmit = async (data: PlayerFormData) => {
    ...
    if (photoFile) {
      ...
      // Si no hay nueva foto pero hay una existente, mantenerla
          ...
          <div className="container mx-auto px-6 py-4">
            <Link href="/players" className="flex items-center space-x-4 hover:opacity-80 transition-opacity">
              ...
              <div>
                ...
                <p className="text-xs text-dark-text-secondary font-light">Jugadores</p>
              </div>
            </Link>
          </div>
        </header>
      ...
      <header className="bg-dark-surface/80 backdrop-blur-xl border-b border-dark-border/50 sticky top-0 z-50">
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <Link href={`/players/${playerId}`} className="flex items-center space-x-4 hover:opacity-80 transition-opacity">
              <div className="w-12 h-12 rounded-2xl bg-success/20 flex items-center justify-center shadow-lg shadow-success/20">
                <ScoutaLogo size="sm" className="text-success" />
              </div>
              <div>
                <h1 className="text-xl font-bold text-white">Scouta</h1>
                <p className="text-xs text-dark-text-secondary font-light">Editar Jugador</p>
              </div>
            </Link>
          </div>
        </div>
        ...
        <div className="bg-dark-surface/80 backdrop-blur-xl border border-dark-border/50 rounded-3xl p-8 shadow-2xl">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex gap-4 pt-4">
              ...
              >
                {isLoading || isUploadingPhoto ? 'Guardando...' : 'Guardar Cambios'}
              </button>
              <Link
                href={`/players/${playerId}`}
                className="px-6 py-3 bg-dark-elevated hover:bg-dark-hover text-white rounded-xl font-semibold transition-all border border-dark-border"
              >
                Cancelar
              </Link>
            </div>
